...
for(let type in optionTypes){
  describe(`Component OrderOption with type=${type}`, () => {
    ...
    switch (type) {
      ...
      case 'icons': {
        it('contains divs and Icon', () => {
          const div = renderedSubcomponent.find('div');
          expect(div.length).toBe(4);   // znajduje cztery, czyli tez ukryty

          const blankValue = div.find('Icon[name="times-circle"]');
          expect(blankValue.length).toBe(1);
